...
export function Ayesha(props) {
  const { nodes, materials } = useGLTF('/models/chars/ayesha.glb')
...
useEffect(() => {
  ...
  return (
    <group {...props} dispose={null} ref={group}  position={[-3,1,0]} rotation={[5,0,0]}  >
         <group onClick={play} rotation={[Math.PI/2,0,0]} position={[0,-0.21,1.3]}  >
        <mesh  >
        <meshBasicMaterial map={texture} color={"grey"}  />
      <planeGeometry args={[0.4,0.07]}  />

        </mesh>
        <Text fontSize={0.04} color={"white"} position={[0,0,0.01]} > Dr.Ayesha Hakim </Text>
      </group>
...
useGLTF.preload('/models/chars/ayesha.glb')